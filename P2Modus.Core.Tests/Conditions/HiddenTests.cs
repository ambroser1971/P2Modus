...
namespace P2Modus.Core.Tests
{
    ...
    public class HiddenTests
    {
        ...
        public void Apply_Character_NoConditions_Test()
        {
            ...
            var hidden = new Hidden();
            hidden.Apply(character);
            Assert.AreEqual(EntityIds.HIDDEN_CONDITION_ID, character.Conditions[0].Id);
            Assert.AreSame(hidden, character.Conditions[0]);
        ...
        public void Apply_Character_WithExistingHiddenCondition_Test()
        {
            var character = CreateMockCharacter();
            character.Conditions.Add(new Hidden());
            var hidden = new Hidden();
            hidden.Apply(character);
        }
